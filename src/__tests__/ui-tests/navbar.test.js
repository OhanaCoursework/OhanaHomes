...
const myoptions = new chrome.Options().headless().addArguments("disable-gpu", "--window-size=2560,1440");
const TIMEOUT = 6000;
let driver;
...
describe("Selenium Tests for NavBar", function () {
  it("Should be able to navigate to the Marketplace Page using the Buy Link", async function () {
    ...
    assert.strictEqual("http://localhost:3000/marketplace", await driver.getCurrentUrl());
  });
  ...
  it("Should be display island links when user hovers over islands button", async function () {
    ...
    const islandsLinks = await driver.findElements(By.className("dropdownLink"));
    ...
    const islandsButton = await driver.findElement(By.xpath('//button[text()="Islands"]'));
  ...
  it("Should be navigate to islands page when island link is clicked", async function () {
    ...
    assert.strictEqual("http://localhost:3000/islands", await driver.getCurrentUrl());
  });
  ...
  it("Should be able to navigate to the About us Page using the About Link", async function () {
    ...
    assert.strictEqual("http://localhost:3000/aboutUs", await driver.getCurrentUrl());
  });
  ...
  it("Should be able to navigate to the Contact Page using the Contact Link", async function () {
    ...
    assert.strictEqual("http://localhost:3000/contact", await driver.getCurrentUrl());
  });
  ...
  it("Should be able to navigate to the Sign Up Page using the Sign Up Link", async function () {
    ...
    assert.strictEqual("http://localhost:3000/signUp", await driver.getCurrentUrl());
  });
  ...
  it("Should be able to navigate to the Login Page using the Login Link", async function () {
    ...
    assert.strictEqual("http://localhost:3000/login", await driver.getCurrentUrl());
  });
  ...
  it("Should be able to navigate to the Home Page by clicking the logo", async function () {
    ...
    await driver.findElement(By.xpath('//*[@class="brandLogoDiv"]/a[1]')).click();
